...
impl<'a> Calendar<'a> {
    ...
    fn selected_date(&self) -> NaiveDate {
        match self.kind {
            CalendarKind::OneWay => self.state.get_one_way_date(),
            CalendarKind::RoundTrip => self.state.get_round_trip_date(),
        }
    }
    ...
    fn min_selectable_date(&self, today: NaiveDate) -> NaiveDate {
        match self.kind {
            ...
            CalendarKind::RoundTrip => self.state.get_one_way_date().succ_opt().unwrap(),
        }
    }
    ...
    fn render_header(&mut self, ui: &mut Ui, today: NaiveDate) {
        let earliest = match self.kind {
            CalendarKind::OneWay => today,
            CalendarKind::RoundTrip => self.state.get_one_way_date(),
        };

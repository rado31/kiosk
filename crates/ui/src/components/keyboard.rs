...
const INPUT_ID: &str = "keyboard_input";
...
fn render_letter_row(
    ...
    ui.horizontal(|ui| {
        ...
        for (i, &ch) in keys.iter().enumerate() {
            ...
            if key_tapped(ui, key_width, KEY_HEIGHT, &ch.to_string()) {
                insert_at_cursor(ctx, value, ch, max_len);
            }
        }
    });
}
...
fn render_row2(
    ...
    ui.horizontal(|ui| {
        ...
        let (rect, response) =
            ui.allocate_exact_size(vec2(backspace_width, KEY_HEIGHT), Sense::CLICK);
...
fn render_space(
    ...
    ui.horizontal(|ui| {
        ...
        if response.clicked() {
            insert_at_cursor(ctx, value, ' ', max_len);
        }
    });
}
...
fn insert_at_cursor(ctx: &Context, value: &mut String, ch: char, max_len: Option<usize>) {
...
fn backspace_at_cursor(ctx: &Context, value: &mut String) {
    let id = Id::new(INPUT_ID);
...
fn key_tapped(ui: &mut Ui, width: f32, height: f32, label: &str) -> bool {
    ...
    ui.painter().text(
        ...
        label,
        FontId::new(KEY_FONT_SIZE, FontFamily::Proportional),
        colors::FG,
    );
